...
#Set up the attack and defense names to use in ranking
attack=glm.fit$model$attack
defend=glm.fit$model$defend
team.names = sort(unique(as.character(attack)))
nteams.in.fit = length(team.names)

#glm coef is sorted alpha by name; each team has an attack and defend score so names are the same in those sets
attack.scores = rep(0,length(glm.fit$xlevels$attack))
names(attack.scores)=glm.fit$xlevels$attack
attacknames = paste("attack",glm.fit$xlevels$attack,sep="")
matchnames = match(attacknames,names(glm.fit$coef))
attack.scores[!is.na(matchnames)]=glm.fit$coef[matchnames[!is.na(matchnames)]]

defense.scores = rep(0,length(glm.fit$xlevels$defend))
names(defense.scores)=glm.fit$xlevels$defend
defendnames = paste("defend",glm.fit$xlevels$defend,sep="")
matchnames = match(defendnames,names(glm.fit$coef))
defense.scores[!is.na(matchnames)]=glm.fit$coef[matchnames[!is.na(matchnames)]]
...
#Compute n.games for each team
n=c()
for(i in team.names){
n[i]=sum(i == as.character(attack))
}
