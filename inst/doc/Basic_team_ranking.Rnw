...
<<rank-teams, keep.source=TRUE, results=hide, eval=FALSE>>=
...
If we want to include explanatory variables in our model, then our scores dataframe also needs columns for the game-specific explanatory variables. In this example, I added surface to the scores dataframe to specify whether the game was played on turf, grass or unknown.  The explanatory variables are specified in the \verb@rank.teams@ call using \verb@add=@.  To include the surface information in our model, add \verb@add="surface"@ to the \verb@rank.teams@ call.
